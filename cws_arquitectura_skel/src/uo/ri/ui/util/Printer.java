...
public class Printer {
	...
	private static void printPaymentMean(PaymentMeanDto medio) {
		Console.printf("\t%s \t%-8.8s \t%s \n", medio.id, medio.getClass().getName(), medio.accumulated);
	}
	...
	public static void printClient(ClientDto c) {
		Console.printf("\t%d %-10.10s %-25.25s %-25.25s %-25.25s %-25.25s %-25.25s %-25.25s %-25.25s\n", 
				c.id, c.dni, c.name, c.surname, c.addressStreet, c.addressCity, c.addressZipcode, c.phone, c.email);
	}
}